...
%start() ->
%    start(new_file_system()). %XXX: fixa en #init eller nåt för att fixa till detta snyggare?
...
file_info(Path) ->
    case gen_server:call(me,{lookup,Path}) of
        {ok, Info} -> 
            case dict:find(nodeInfo,Info) of
                {ok, NodeInfo} -> 
...
new_file(FileName) ->
    ...
    Attrs= [
        {name, FileName},
        {type, regular}, 
        {inode, get_or_create_inode_number_for(FileName)},
...
get_or_create_inode_number_for(Filename,Path) ->
