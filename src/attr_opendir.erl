...
direntries(Inode) ->
  ...
  ?DEBL(9,"Converting children ~p for ~p",[Children,Inode]),
  direntrify(Children).
...
direntrify([{Name,Inode,_Type}|Children],N) ->
  ?DEB2(9,"Getting entry for child ~p",{Name,Inode,_Type}),
  {value,Child}=tree_srv:lookup(Inode,inodes),
  ?DEB2(9,"Getting permissions for child ~p",{Name,Inode,_Type}),
  ChildStats=Child#inode_entry.stat,
  ?DEB2(9,"Creating direntry for child ~p",{Name,Inode,_Type}),
  Direntry= #direntry{name=Name ,stat=ChildStats },
  ?DEB2(9,"Calculatig size for direntry for child ~p",{Name,Inode,_Type}),
