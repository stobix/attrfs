...
remove_attribute(Path,Inode,Attribute) ->
  ...
  case length(Matches)>0 of
    true ->
      ?DEBL({remove,3},"Removing ~p for ~p from database", [Attribute,Path]),
      dets:match_delete(?ATTR_DB,{Path,Attribute});
    false ->
      ?DEB1({remove,3},"No data base entry to remove!")
  end,
...
remove_key_values(Path,Inode,AName) ->
  ?DEBL({remove,3},"deleting ~p from ~p",[AName,Path]),
  % Database handling
  Matches=dets:match(?ATTR_DB,{Path,['$1'|AName]}),
  case length(Matches)>0 of
    true -> 
      ?DEBL({remove,5},"removing the following items (if any): ~p",[Matches]),
      dets:match_delete(?ATTR_DB,{Path,['_'|AName]});
    false -> 
      ?DEB1({remove,5},"found no items to remove, doing nothing"),
      ok
  end,
  ?DEBL({remove,5},"items left (should be none): ~p",[dets:match(?ATTR_DB,{Path,['$1'|AName]})]),
...
remove_empty_dir(ParentIno,DirName) ->
  ?DEB1({remove,3},">remove_empty_dir"),
  ...
  case lists:keytake(DirName,1,ParentEntry#inode_entry.contents) of
    {value,{_DeletedChild,ChildIno,_ChildType},NewChildren} ->
      ?DEBL({remove,5},"removing empty dir ~p from parent ~p",[DirName,ParentIno]),
    ...
    _ -> 
      ...
      ?DEB1({remove,5},"dir not a child of parent! not removing!!"),
      enoent
  end.
